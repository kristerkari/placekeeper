setTimeout(function() {
  ...
  setTimeout(function() {
    ...
    window.alert($("p1").getValue());
    window.alert($("p2").getValue());
    window.alert($("handle1").getValue());
    window.alert($("handle2").getValue());
    window.alert($("handle3").getValue());

    // The behaviour of `getValue` as a static method should be patched
    window.alert(Form.Element.getValue($("p1")));
